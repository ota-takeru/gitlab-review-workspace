...
<template><div class="gl-diff-table" role="table" :aria-label="props.ariaLabel ?? props['aria-label'] ?? 'Diff'"><slot name="header" /><template v-for="(line,index) in lines" :key="line.key ?? `${index}:${line.oldLine ?? ''}:${line.newLine ?? ''}`"><slot name="line" :line="line" :index="index"><div class="gl-diff-row" :class="line.kind" role="row"><span class="gl-diff-gutter" role="cell">{{ line.oldLine ?? '' }}</span><span class="gl-diff-gutter" role="cell">{{ line.newLine ?? '' }}</span><span class="gl-diff-marker" role="cell" aria-hidden="true">{{ markerFor(line) }}</span><code class="gl-diff-code" role="cell">{{ codeFor(line) || ' ' }}</code></div></slot></template></div></template>
<style scoped>
